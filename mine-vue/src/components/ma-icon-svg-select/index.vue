<template>
  <span>
    <el-popover
      ...
      <el-row class="ma-icon-svg-select--group">
        <el-col
          v-for="(item, index) in iconFilted"
          :key="index"
          class="ma-icon-svg-select--item"
          :span="4"
          @click.native="selectIcon(item)">
          <ma-icon-svg :name="item"/>
        </el-col>
      </el-row>
    </el-popover>
    <!-- 允许用户输入 -->
    <el-input
      ...
      <template v-if="value" slot="prepend">
        <ma-icon-svg
          class="ma-icon-svg-select--input-preview"
          :name="value"/>
      </template>
    ...
    <el-button v-popover:pop v-if="!userInput">
      <span flex="dir:left main:center cross:center">
        <ma-icon-svg
          v-if="value"
          class="ma-icon-svg-select--input-preview ma-mr-10"
          :name="value"/>
...
.ma-icon-svg-select--input-preview {
  height: 14px;
  width: 14px;
  display: block;
}
.ma-icon-svg-select--group {
  max-height: 400px;
  overflow-x: hidden;
  overflow-y: scroll;
  margin-top: 10px;
}
.ma-icon-svg-select--item {
  height: 40px;
  display: flex;
